...
# Implementation of methods {.tabset}
...
DT::datatable(results %>% sample_n(size=100), rownames = FALSE, options=list(scrollX=TRUE, autoWidth = TRUE))
```
...
DT::datatable(shapes %>% sample_n(size=100) %>% dplyr::select(-geometry), rownames = FALSE, options=list(scrollX=TRUE, autoWidth = TRUE))
```
...
```{r power_plot, out.extra='.preview-image'}
ggplot(
    tibble(
