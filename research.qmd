...
def button(url, str, icon):
    icon_base = icon[:2]
    return f"""<a class="btn btn-outline-dark btn-sm", href="{url}">
...
for _, data in yaml_data.items():
    ...
    if working_paper:
        ...
        pub_strs["wps"][year_str].append("<li>" + pub_str + "<br>" + " ".join(buttons) + "</li>")
    else:
        if year_str not in pub_strs["pubs"]:
            pub_strs["pubs"][year_str] = []
        buttons.append(button(pub_url, "Published", "ai-archive"))
        pub_strs["pubs"][year_str].append("<li>" + pub_str + "<br>" + " ".join(buttons) + "</li>")
